...
import { QuestionsDto2, OptionsDto2, QuestionTestDto, OptionTestDto } from '../../../core/api/clients';
...
export class QuestionCarde implements OnChanges {
  ...
  @Input({ required: true }) question!: QuestionsDto2 | QuestionTestDto | any;
  ...
  getOptionClass(option: any): string {
    ...
    if (!this.selectedOptionId) return 'bg-white border-gray-200 text-gray-700 hover:border-indigo-400';
    ...
    if (typeof option.isCorrect === 'undefined') {
      if (isSelected) {
  ...
  // دالة مساعدة للـ HTML لمعرفة هل نحن في وضع اختبار أم لا
  isTestMode(option: any): boolean {
    return typeof option.isCorrect === 'undefined';
  }
}
